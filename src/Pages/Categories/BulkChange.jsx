...
export function SearchWithDropdownSelector({ handleSelectedProducts }) {
      ...
      {selectedProducts.length > 0 && (
        <ul>
          {selectedProducts.map((product) => (
            <span
              key={product._id}
              className="inline-flex items-center pl-3 py-0.5 rounded-full text-sm font-medium bg-indigo-100 text-indigo-800 mr-2 mb-1"
            >
              {product.name}
              <button
                type="button"
      ...
      <Combobox.Options className="mt-1 absolute z-50 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
        {searchResult.map((product) => (
          <Combobox.Option
            ...
            value={product}
            className={classNames(
              "cursor-default select-none relative py-2 px-4 hover:bg-indigo-100",
